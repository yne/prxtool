...
void elf_dumpSymbols(ElfCtx*elf,FILE*stream){
	...
	if(!elf->symbolsCount)
		return (void)fprintf(stream,"There is no symbols in this file.");
	for(ElfSymbol*s=elf->symbols;s < elf->symbols+elf->symbolsCount; s++)
...
void elf_dumpPrograms(ElfCtx*elf,FILE*stream){
	...
	if(!elf->iPHCount)
		return (void)fprintf(stream,"There is no programs in this file.");
...
void elf_dumpSections(ElfCtx*elf,FILE*stream){
	...
	if(!elf->iSHCount)
		return (void)fprintf(stream,"There is no sections in this file.");
